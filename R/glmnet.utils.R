...
impute.glmnet.matrix_fit = function (x, n_cores = 1) {
  ...
  m = foreach(j=1:ncol(x), .export=c('assign.folds','.assign.folds_one_site','cv.glmnet','glmnet.control','glmnet','glmnet_predict','glmnet_fit','.assign.folds_simple'), .options.snow = opts) %dopar% {
  ...
  #for (j in 1:ncol(x)) {
    ...
    for (k in seq_len(length(completeCols))) {
      # cat(paste0(round(j / ncol(x) * 100), '% completed'))
  ...
  close(pb)
  stopCluster(cl)
  cat("[impute.glmnet.matrix_fit] Running time:", difftime(Sys.time(), start.time, units = "mins"), "minutes\n")
...
impute.glmnet.matrix = function (m, x, nimp = 20) {
  ...
  for (j in seq_len(ncol(x))) {
    ...
    # if (j == ncol(x)) cat(': Done')
    # else cat('\014')
  }
  cat("[impute.glmnet.matrix] Running time:", difftime(Sys.time(), start.time, units = "mins"), "minutes\n")
  x.imp
}
