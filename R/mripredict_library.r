...
.load_mri = function (mri_paths, mask=NULL , space = "MNI") {
  #if(!is.null(mask) && mask=="") mask=NULL
  mri = c()
  if(is.numeric(mri_paths) && length(dim(mri_paths))==4){ # if mri_paths contains already valid 4d data
    mri$data <- mri_paths
  ...
  } else { # if mri_paths are a list of paths
    n = length(mri_paths);
    if (!n) {
...
.load_mri_old = function (mri_paths, space = 'MNI') {
  n = length(mri_paths);
  if (!n) {
    stop("No MRI data have been specified")
  }
  ...
  for (i in 1:n) {
    ...
    if (mri_i$scl.slope || (mri_i$scl.inter || mri_i$scl.slope != 1)) {
      mri$data[,,,i] = mri_i$scl.inter + mri_i$scl.slope * mri_i$data
    } else {
      mri$data[,,,i] = mri_i$data
    }
...
.most_frequent_variables <- function(model_list, mp = NULL, file=NULL) {
  ...
  betes = out
  resum_betes=do.call(data.frame, aggregate(betes$betas ~ betes$name, data = betes, FUN = function(x) c(sum = sum(x), n = length(x) ) ) )
  colnames(resum_betes) = c('variable', 'beta_sum','beta_count')
  
  if(!is.null(file)) {
    write.csv(resum_betes, file=gsub(".csv", "_summary.csv",file))
...
.most_frequent_variables_ijk <- function(model_list, mp = NULL, file=NULL) {
  ...
  for(i in seq_len(length(model_list))) {
    ...
    # tenir en compte cas en què al model no hi ha mri
...
.metrics_binary = function (real, predictions, folder="", save=FALSE) {
  ...
  metric <- data.frame(class1 = class1,
                       ...
                       npv = round(tn/(tn+fn), digits=3),
                       sensitivity = round(acc_class1, digits=3),
                       specificity = round(acc_class2, digits=3),
                       bac = round(bac, digits=3))
  if (save)
