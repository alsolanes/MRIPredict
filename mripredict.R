...
cut_brain <-
function (brain, degrees = 0, axe = 1) 
{
    ...
    for (i in 1:size_brain[1]) {
        for (j in 1:size_brain[2]) {
            for (k in 1:size_brain[3]) {
                coords = rotate_coordinates(degrees, axe = axe, 
                  coordinates = c(i, j, k), translate_xyz = translate_XYZ) + 
                  1.5 * size_brain
                if (coords[axe] >= z_cut) {
                  brain[i, j, k] = FALSE
                }
...
cut_brain_mc <-
function (brain, degrees = 0, axe = 1, n_cores = 2) 
{
    size_brain = dim(brain)
    z_cut = split_brain(brain, degrees, axe)$z_cut
    translate_XYZ = round(c(size_brain/2))
    brain2 = pbmclapply(expand.grid(i = 1:size_brain[1], j = 1:size_brain[2], 
        k = 1:size_brain[3]), function(brain) {
        coords = rotate_coordinates(degrees, axe = axe, coordinates = c(i, 
            j, k), translate_xyz = translate_XYZ) + 1.5 * size_brain
        if (coords[axe] >= z_cut) {
            brain[i, j, k] = FALSE
        }
        brain
...
get_rotated_coordinates <-
function (degrees, axe, coordinates = c(i, j, k), brain) 
{
    size_brain = dim(brain)
    rotate_coordinates(degrees, axe = axe, coordinates = coordinates, 
        translate_xyz = round(c(dim(brain)/2))) + 1.5 * size_brain
}
glmnet_fit <-
...
index_median <-
function (brain, axe = 3) 
{
    z_median = unlist(lapply(1:dim(brain)[axe], function(i) {
        switch(as.character(axe), `1` = sum(brain[i, , ]), `2` = sum(brain[, 
            i, ]), `3` = sum(brain[, , i]))
    }))
...
function (brain) 
{
    ...
    M = melt(brain > 0.001)
    M = M[M$value == TRUE, ]
    plot3d(M$Var1, M$Var2, M$Var3, size = 10, transparency = 0.5, 
        xlab = "X", ylab = "Y", zlab = "Z")
}
plot_Brain2D <-
function (X, y_cut = round(dim(X)[2]/2)) 
{
    if (y_cut == "") 
        image.default(X)
    else image.default(X[, y_cut, ])
    dev.off()
}
postmean <-
function (g.hat, g.bar, n, d.star, t2) 
...
rotate_brain <-
function (brain, degrees, axe = 3) 
{
    size_brain = dim(brain)
    rotated_brain = array(rep(0, prod(size_brain * 3)), dim = size_brain * 
        3)
    for (i in 1:size_brain[1]) {
        for (j in 1:size_brain[2]) {
            for (k in 1:size_brain[3]) {
                if (brain[i, j, k] == TRUE) {
                  rotated = rotate_coordinates(degrees, axe = axe, 
                    coordinates = c(i, j, k), translate_xyz = round(c(dim(brain)/2))) + 
                    size_brain * 1.5
...
rotate_brain_mc <-
function (brain, degrees, axe = 3, n_cores = 2) 
{
    size_brain = dim(brain)
    rotated_brain = array(rep(0, prod(size_brain * 3)), dim = size_brain * 
        3)
...
rotate_coordinates_old <-
function (degrees, axe = 1, coordinates, translate_xyz) 
{
    translate_matrix_3d(translate_xyz[1], translate_xyz[2], translate_xyz[3]) %*% 
        rotation_matrix_3d(degrees)[[axe]] %*% translate_matrix_3d(-translate_xyz[1], 
        -translate_xyz[2], -translate_xyz[3]) %*% matrix(c(coordinates, 
        1), ncol = 1)
}
rotation_matrix_3d <-
function (degrees) 
