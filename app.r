...
server <- function(input, output, session) {
  ...
  observeEvent(input$confirmFit, {
    ...
    paths = cbind(mri_files$paths_gm_un, mri_files$paths_gm_fu, mri_files$paths_wm_un, mri_files$paths_wm_fu)
    mp <- mripredict(mri_data = paths, data_table_file = mri_files$clinical, response_var = mri_files$response, 
  ...
  observeEvent(input$confirmPredict, {
    ...
    withProgress(message = "Calculating predictions", value = 0, {
      results <- mripredict_predict(mp = mp, mri_data = paths, data_table_file = mri_files$clinical, space='NO_CHECK', n_cores=1)
      # if(mp$modulation == 'fu'){
      #   mp <- mripredict_predict(mp = mp, mri_data = mri_files$paths_gm_un, mri_fu_paths_file = mri_files$paths_gm_fu, data_table_file = mri_files$clinical, space='NO_CHECK', n_cores=1)
      # } else {
      #   mp <- mripredict_predict(mp = mp, mri_data = paths, data_table_file = mri_files$clinical, space='NO_CHECK', n_cores=1)
      # }
    })
